...
<?php include "model/timesheetAdminDAO.php" ?>
<?php include "model/createDTRDAO.php" ?>
<?php include "model/timesheetDAO.php" ?>
<?php include "DigitalClock.php" ?>
...
class ctbl_employee_list extends ctbl_employee {
	...
	function Page_Main() {
		...
		if ($this->IsPageRequest()) { // Validate request
			...
			// Set up Breadcrumb
			$this->SetupBreadcrumb(); //cmalvarez
...
?>
<form action = "dtr_edit_n.php" method="GET">
<br/><br/><br/>
	...
	<tr>
		...
		foreach($getDTRofMonth as $keyDTRMo => $valDTRMo)
		{
			...
			*/
			...
			echo "<td>";
			echo "<a href =dtr_edit_n.php?emp_id=".$valDTRMo['emp_id']."&dtr_id=".$valDTRMo['dtr_id']." />Edit</a>";
			echo "</td>";
			...
			/*if($valDTRMo['day'] <=9)
			{
				...
				$dateTodayLogNew = $valDTRMo['year'] . "-" . $valDTRMo['month'] ."-0".$valDTRMo['day'];
			}else
			{
				$dateTodayLogNew = $valDTRMo['year'] . "-" . $valDTRMo['month'] ."-".$valDTRMo['day'];
			}*/
			//echo $dateTodayLogNew;
			//echo $dateTodayLogNew;
