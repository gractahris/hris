...
$tbl_employee_deduction_grid = NULL; // Initialize page object first

class ctbl_employee_deduction_grid extends ctbl_employee_deduction {
	...
	// Page object name
	var $PageObjName = 'tbl_employee_deduction_grid';

	// Grid form hidden field names
	var $FormName = 'ftbl_employee_deductiongrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["tbl_employee_deduction"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
		...
		for ($rowindex = 1; $rowindex <= $rowcnt; $rowindex++) {
			...
			if (!$this->EmptyRow()) {
				...
				if ($bGridInsert) {
					...
					$sKey .= $this->deduction_ref_id->CurrentValue;
	...
	function EmptyRow() {
		global $objForm;
		if ($objForm->HasValue("x_deduction_id") && $objForm->HasValue("o_deduction_id") && $this->deduction_id->CurrentValue <> $this->deduction_id->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_deduction_amount") && $objForm->HasValue("o_deduction_amount") && $this->deduction_amount->CurrentValue <> $this->deduction_amount->OldValue)
			return FALSE;
	...
	function RenderListOptions() {
		...
		if ($this->CurrentMode == "edit" && is_numeric($this->RowIndex)) {
			$this->MultiSelectKey .= "<input type=\"hidden\" name=\"" . $KeyName . "\" id=\"" . $KeyName . "\" value=\"" . $this->deduction_ref_id->CurrentValue . "\">";
		}
	...
	function SetRecordKey(&$key, $rs) {
		...
		$key .= $rs->fields('deduction_ref_id');
	}
	...
	function LoadDefaultValues() {
		$this->deduction_ref_id->CurrentValue = NULL;
		$this->deduction_ref_id->OldValue = $this->deduction_ref_id->CurrentValue;
	...
	function LoadFormValues() {
		...
		// Load from form
		global $objForm;
		if (!$this->deduction_ref_id->FldIsDetailKey && $this->CurrentAction <> "gridadd" && $this->CurrentAction <> "add")
		...
		$this->deduction_id->setOldValue($objForm->GetValue("o_deduction_id"));
		if (!$this->deduction_amount->FldIsDetailKey) {
			$this->deduction_amount->setFormValue($objForm->GetValue("x_deduction_amount"));
		}
		$this->deduction_amount->setOldValue($objForm->GetValue("o_deduction_amount"));
	...
	function LoadOldRecord() {
		...
		if ($cnt >= 1) {
			if (strval($arKeys[0]) <> "")
				$this->deduction_ref_id->CurrentValue = strval($arKeys[0]); // deduction_ref_id
			else
				$bValidKey = FALSE;
		} else {
			$bValidKey = FALSE;
