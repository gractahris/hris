...
<?php
				...
				$timeInClock = $clock->jsDigitalClock(300, 300);
				echo $timeInClock;
?>				
			</td>
		</tr>
...
foreach($monthArr as $key => $val)
{
	...
	if($clock->getMonth() == $val)
	{
		...
		if($counter < 10)
		{
			...
			$getDTR = $timesheetDAO->getDTR($_SESSION['emp_id'],$month,$clock->getDay(),$clock->getYear());
			if($getDTR == true)
			{
				foreach($getDTR as  $keyDTR => $valDTR)
				{
					...
					?>
					<td>
					<input class ="btn btn-success btn-xl" type="submit" id ="timeIN" name="timeIn" value="Time In" />
					</td>
					<td>
					<input type="submit" id ="timeOut" name="timeOut" value="Time Out" />
					</td>
					</form>
					<?php
					
					if($_GET["timeIn"] == "timeIn")
					{
...
$getDTRofMonth = $timesheetDAO->getDTRofMonth($_SESSION['emp_id'],$month_val, $clock->getYear());
...
<?php
	foreach($getDTRofMonth as $keyDTRMo => $valDTRMo)
	{
		...
		*/
		...
		echo "<td>";
		echo $getTimeLogOfDay[0]["emp_timein"];
		foreach($getTimeLogOfDay as $keyLog => $valLog)
		{
			
			//echo $valLog['emp_timein'];
		}
		echo "</td>";
			
		echo "<td>";
		echo $getTimeLogOfDay[0]["emp_timeout"];
		echo "</td>";
