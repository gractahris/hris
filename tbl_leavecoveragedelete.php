...
$tbl_leavecoverage_delete = NULL; // Initialize page object first

class ctbl_leavecoverage_delete extends ctbl_leavecoverage {
	...
	// Page object name
	var $PageObjName = 'tbl_leavecoverage_delete';
	...
	function Page_Main() {
		...
		if ($sFilter == "")
			$this->Page_Terminate("tbl_leavecoveragelist.php"); // Prevent SQL injection, return to list

		// Set up filter (SQL WHHERE clause) and get return SQL
		// SQL constructor in tbl_leavecoverage class, tbl_leavecoverageinfo.php
...
// Create page object
if (!isset($tbl_leavecoverage_delete)) $tbl_leavecoverage_delete = new ctbl_leavecoverage_delete();

// Page init
$tbl_leavecoverage_delete->Page_Init();

// Page main
$tbl_leavecoverage_delete->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$tbl_leavecoverage_delete->Page_Render();
?>
<?php include_once "header.php" ?>
<script type="text/javascript">

// Page object
var tbl_leavecoverage_delete = new ew_Page("tbl_leavecoverage_delete");
tbl_leavecoverage_delete.PageID = "delete"; // Page ID
var EW_PAGE_ID = tbl_leavecoverage_delete.PageID; // For backward compatibility

// Form object
var ftbl_leavecoveragedelete = new ew_Form("ftbl_leavecoveragedelete");

// Form_CustomValidate event
ftbl_leavecoveragedelete.Form_CustomValidate = 
...
ftbl_leavecoveragedelete.ValidateRequired = true;
<?php } else { ?>
ftbl_leavecoveragedelete.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
ftbl_leavecoveragedelete.Lists["x_emp_id"] = {"LinkField":"x_emp_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_empLastName","x_empFirstName","x_empMiddleName",""],"ParentFields":[],"FilterFields":[],"Options":[]};
...
if ($tbl_leavecoverage_delete->Recordset = $tbl_leavecoverage_delete->LoadRecordset())
	$tbl_leavecoverage_deleteTotalRecs = $tbl_leavecoverage_delete->Recordset->RecordCount(); // Get record count
if ($tbl_leavecoverage_deleteTotalRecs <= 0) { // No record found, exit
	if ($tbl_leavecoverage_delete->Recordset)
		$tbl_leavecoverage_delete->Recordset->Close();
	$tbl_leavecoverage_delete->Page_Terminate("tbl_leavecoveragelist.php"); // Return to list
}
?>
<?php $Breadcrumb->Render(); ?>
<?php $tbl_leavecoverage_delete->ShowPageHeader(); ?>
<?php
$tbl_leavecoverage_delete->ShowMessage();
?>
<form name="ftbl_leavecoveragedelete" id="ftbl_leavecoveragedelete" class="ewForm form-horizontal" action="<?php echo ew_CurrentPage() ?>" method="post">
<input type="hidden" name="t" value="tbl_leavecoverage">
<input type="hidden" name="a_delete" id="a_delete" value="D">
<?php foreach ($tbl_leavecoverage_delete->RecKeys as $key) { ?>
<?php $keyvalue = is_array($key) ? implode($EW_COMPOSITE_KEY_SEPARATOR, $key) : $key; ?>
<input type="hidden" name="key_m[]" value="<?php echo ew_HtmlEncode($keyvalue) ?>">
<?php } ?>
<table cellspacing="0" class="ewGrid"><tr><td class="ewGridContent">
<div class="ewGridMiddlePanel">
<table id="tbl_tbl_leavecoveragedelete" class="ewTable ewTableSeparate">
<?php echo $tbl_leavecoverage->TableCustomInnerHtml ?>
	<thead>
	<tr class="ewTableHeader">
<?php if ($tbl_leavecoverage->leave_coverage_id->Visible) { // leave_coverage_id ?>
		<td><span id="elh_tbl_leavecoverage_leave_coverage_id" class="tbl_leavecoverage_leave_coverage_id"><?php echo $tbl_leavecoverage->leave_coverage_id->FldCaption() ?></span></td>
<?php } ?>
<?php if ($tbl_leavecoverage->leave_application_id->Visible) { // leave_application_id ?>
		<td><span id="elh_tbl_leavecoverage_leave_application_id" class="tbl_leavecoverage_leave_application_id"><?php echo $tbl_leavecoverage->leave_application_id->FldCaption() ?></span></td>
<?php } ?>
<?php if ($tbl_leavecoverage->emp_id->Visible) { // emp_id ?>
		<td><span id="elh_tbl_leavecoverage_emp_id" class="tbl_leavecoverage_emp_id"><?php echo $tbl_leavecoverage->emp_id->FldCaption() ?></span></td>
<?php } ?>
<?php if ($tbl_leavecoverage->date_from->Visible) { // date_from ?>
		<td><span id="elh_tbl_leavecoverage_date_from" class="tbl_leavecoverage_date_from"><?php echo $tbl_leavecoverage->date_from->FldCaption() ?></span></td>
<?php } ?>
<?php if ($tbl_leavecoverage->date_to->Visible) { // date_to ?>
		<td><span id="elh_tbl_leavecoverage_date_to" class="tbl_leavecoverage_date_to"><?php echo $tbl_leavecoverage->date_to->FldCaption() ?></span></td>
<?php } ?>
	</tr>
	</thead>
	<tbody>
<?php
$tbl_leavecoverage_delete->RecCnt = 0;
$i = 0;
while (!$tbl_leavecoverage_delete->Recordset->EOF) {
	$tbl_leavecoverage_delete->RecCnt++;
	$tbl_leavecoverage_delete->RowCnt++;

	// Set row properties
	$tbl_leavecoverage->ResetAttrs();
	$tbl_leavecoverage->RowType = EW_ROWTYPE_VIEW; // View

	// Get the field contents
	$tbl_leavecoverage_delete->LoadRowValues($tbl_leavecoverage_delete->Recordset);

	// Render row
	$tbl_leavecoverage_delete->RenderRow();
?>
	<tr<?php echo $tbl_leavecoverage->RowAttributes() ?>>
<?php if ($tbl_leavecoverage->leave_coverage_id->Visible) { // leave_coverage_id ?>
		<td<?php echo $tbl_leavecoverage->leave_coverage_id->CellAttributes() ?>>
<span id="el<?php echo $tbl_leavecoverage_delete->RowCnt ?>_tbl_leavecoverage_leave_coverage_id" class="control-group tbl_leavecoverage_leave_coverage_id">
...
<span id="el<?php echo $tbl_leavecoverage_delete->RowCnt ?>_tbl_leavecoverage_leave_application_id" class="control-group tbl_leavecoverage_leave_application_id">
...
<span id="el<?php echo $tbl_leavecoverage_delete->RowCnt ?>_tbl_leavecoverage_emp_id" class="control-group tbl_leavecoverage_emp_id">
...
<span id="el<?php echo $tbl_leavecoverage_delete->RowCnt ?>_tbl_leavecoverage_date_from" class="control-group tbl_leavecoverage_date_from">
...
<span id="el<?php echo $tbl_leavecoverage_delete->RowCnt ?>_tbl_leavecoverage_date_to" class="control-group tbl_leavecoverage_date_to">
...
<?php
	$tbl_leavecoverage_delete->Recordset->MoveNext();
}
$tbl_leavecoverage_delete->Recordset->Close();
?>
</tbody>
...
<script type="text/javascript">
ftbl_leavecoveragedelete.Init();
</script>
<?php
$tbl_leavecoverage_delete->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$tbl_leavecoverage_delete->Page_Terminate();
?>
