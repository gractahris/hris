...
<?php include "model/leaveApplicationDAO.php" ?>
...
<form name = "EmpName" action="leave_application_list.php" method="GET">
<table border="1" width="100%" class ="table table-striped table-bordered table-hover">
	<tr>
		...
		<?php
		$getAllEmp = $leaveApplicationDAO->getAllEmp();
		foreach($getAllEmp as $keyAllEmp => $valAllEmp)
		{
			$valAllEmpID = $valAllEmp['emp_id'];
			$valAllEmpFullname = strtoupper($valAllEmp['empLastName'] .", ".$valAllEmp['empFirstName']." ".$valAllEmp['empMiddleName']);
			
			echo "<option id=";
			echo $valAllEmpID;
			echo " value=";
			echo $valAllEmpID;
			echo " >".$valAllEmpFullname . "</option>";
		}
	...
	<tr>
		...
		<td>
		<a class = "icon-legal btn btn-sm btn-info" href="leave_application_list.php?leave=1">&nbsp;&nbsp;Pending Leave</a>
		&nbsp;&nbsp;&nbsp;
		<a class = "icon-ok btn btn-sm btn-success" href="leave_application_list.php?leave=2">&nbsp;&nbsp;Approved Leave</a>
		&nbsp;&nbsp;&nbsp;
		<a class = "icon-eye-close btn btn-sm btn-danger" href="leave_application_list.php?leave=3">&nbsp;&nbsp;Disapproved Leave</a>
		</td>
	</tr>
	...
	<a href="leave_application_list.php">Show All</a>
	</td>
	<tr>
	</tr>
</table>
...
?>
<form name="leaveApproved" action="leave_application_list.php" method="GET">
<table border="1" width="100%" class ="table table-striped table-bordered table-hover">
	
	<tr>
		<td></td>
		<td>Leave<br/>Application<br/>ID</td>
		<td></td>
		<td>Employee</td>
		<td>LeaveType</td>
		<td>Days of Leave</td>
		<td>Leave Date Applied</td>
		<td>Leave Coverage From</td>
		<td>Leave Coverage To</td>
		<td>Status</td>
	</tr>
	<tr>
...
if( $leave < 1 && $emp_id == "" || $emp_id == "#")
{
	// $getLeavePerEMP = $leaveApplicationDAO->getLeavePerAllEMP();
	echo "<center>";
	include "pagination_leave.php";
	// $getLeavePerEMP = $leaveApplicationDAO->getLeavePerAllEMP();
	echo "</center>";
	echo "<br/>";
	// echo "BBBBBBBBBBBB";
...
{
	// $getLeavePerEMP = $leaveApplicationDAO->getLeavePending();
	echo "<center>";
	include "pagination_pendingleave.php";
	echo "</center>";
	echo "<br/>";
	// echo "Aaaaaaaa";
}
elseif($leave == 2)
{
	// $getLeavePerEMP = $leaveApplicationDAO->getLeaveApproved();
	echo "<center>";
	include "pagination_apleave.php";
	echo "</center>";
	echo "<br/>";
	// echo "Aaaaaaaa";
}
elseif($leave == 3)
{
	// $getLeavePerEMP = $leaveApplicationDAO->getLeaveDisapproved();
	echo "<center>";
	include "pagination_dpleave.php";
	echo "</center>";
	echo "<br/>";
	// echo "Aaaaaaaa";
}
...
{
	// $getLeavePerEMP = $leaveApplicationDAO->getLeavePerEMP($emp_id);
	echo "<center>";
	include "pagination_leaveByEmp.php";
	echo "</center>";
	echo "<br/>";
}
// print_r($getLeavePerEMP);
foreach($getLeavePerEMP as $key=>$val)
{
	$leave_application_id = $val['leave_application_id'];
	$purpose = $val['sickness'];
	$leave_type_id = $val['leave_type_id'];
	$days_to_leave = $val['days_to_leave'];
	$status_id = $val['status_id'];
	$emp_id_val = $val['emp_id'];
	$date_applied = $val['date_applied'];

	echo "<td>";
	// echo "<a href = 'leave_application_u_edit.php?leave_application_id=".$leave_application_id."' class='icon-edit icon-only'></a>";
	echo "</td>";
	...
	echo "<td>";
	// echo "<input type='submit' name='approve' id='approve' value = 'Approve' />";
	
	// if($status_id == 2) //approved
	if($status_id == 2) //approved
	{	
		//echo $status_id;
		echo "<a' class = 'icon-ok btn btn-sm btn-success' href='leave_application_list.php?emp_id=$emp_id&btn=2&lai=$leave_application_id'>&nbsp;&nbsp;Approve</a>";
		echo "<br/>";
	}else if($status_id == 3)//disapproved
	{
		// echo $status_id;
	echo "<a class ='icon-eye-close btn btn-sm btn-danger' href='leave_application_list.php?emp_id=$emp_id&btn=3&lai=$leave_application_id'>&nbsp;&nbsp;Disapprove</a>";
	
	}else if($status_id == 1)//pending
	{
		// echo $status_id;
	echo "<a class = 'icon-ok btn btn-sm btn-success' href='leave_application_list.php?emp_id=$emp_id&btn=2&lai=$leave_application_id'>&nbsp;&nbsp;Approve</a>";
		echo "<br/>";
	...
	echo "</td>";
	$getEmpByID = $leaveApplicationDAO->getEmpByID($emp_id_val);
	// $getEmpByID = $leaveApplicationDAO->getAllEmp();
	echo "<td>";
	foreach($getEmpByID as $keyEmp => $valEmp)
	{
	...
	echo "<td>";
	$getLeaveType = $leaveApplicationDAO->getLeaveType($leave_type_id);
	foreach($getLeaveType as $keyLT => $valLT)
	{

		$leave_type_title = $valLT['leave_type_title'];
		echo $leave_type_title;
	...
	echo "<td>";
	// echo $date_applied;
	echo date("F d, Y",strtotime($date_applied));
	echo "</td>";


	// echo "<td>";
	// $getLeaveCoverage = $leaveApplicationDAO->getLeaveCoverage($leave_application_id);
	// foreach($getLeaveCoverage as $keyLC => $valLC)
	// {
		// $daysFrom = $valLC['date_from'];
		// echo $daysFrom;
		
	// }
	// echo "</td>";
	
	$getMaxDateTo = $leaveApplicationDAO->getMaxDateTo($leave_application_id);
	
	foreach($getMaxDateTo as $keyMaxTo => $valMaxTo)
	{	
		echo "<td>";
		$daysFrom = $valMaxTo['minDateFrom'];
		// echo $daysFrom;
		echo date("F d, Y",strtotime($daysFrom));
		echo "</td>";
		
		echo "<td>";
		$daysTo = $valMaxTo['maxDateTo'];
		echo date("F d, Y",strtotime($daysTo));
		// echo $daysTo;
		echo "</td>";
	}
	...
	echo "<td>";
	// echo $status_id;
	$getStatus = $leaveApplicationDAO->getStatus($status_id);
	// $print_r($getStatus);
	foreach ($getStatus as $keyStatus => $valStatus) {

		$status_title = $valStatus['status_title'];
		echo $status_title;
	}
...
$timesheetDAO = new timesheetDAO();
	$getLeaveCreditByEmp = $leaveCreditDAO->getLeaveCreditByEmp($emp_id,$leave_type_id);
	 // print_r($getLeaveCreditByEmp);
$status_id_val = $_GET['btn'];
$lai = $_GET['lai'];
$getEmpID = $_GET['emp_id'];
if($leave_application_id <> NULL)
{
	$updateLeaveApplication = $leaveApplicationDAO->updateLeaveApplication($lai, $status_id_val,$leave_type_id);
	if($getLeaveCreditByEmp <> NULL)
	{
		foreach($getLeaveCreditByEmp as $key=> $val)
		{
			// $getLeaveTypeByID = $leaveCreditDAO->getLeaveTypeByID($leave_type_id);
			$getLeaveTypeByID = $leaveCreditDAO->getLeaveTypeByID($lai);
			
			// if($status_id == 3)
			// {
				//checkMe();

				$leaveApplicationDAO->updateLeaveApplication($lai, $status_id_val,$leave_type_id);
			...
			// }else{
				$leave_balance = $val['leave_credit'] - $days_to_leave;
				if($status_id_val == "2")
				{
					...
					$pageWasRefreshed = isset($_SERVER['HTTP_CACHE_CONTROL']) && $_SERVER['HTTP_CACHE_CONTROL'] === 'max-age=0';
 						?>
							...
							window.location.href = "leave_application_list.php";
							// location.reload();
							</script>
							<?php
						...
						// if($pageWasRefreshed ) {
						  //do something because page was refreshed;
							// echo "refressshhhhh";
							$leaveApplicationDAO->updateLeaveCredit($getEmpID, $leave_type_id,$leave_balance);
							$leaveApplicationDAO->updateLeaveApplication($lai, $status_id_val,$leave_type_id);
				...
				echo "<br/>";
				// $getLeaveCoverage = $leaveApplicationDAO->getLeaveCoverage($leave_application_id);
				$getLeaveCoverage = $leaveApplicationDAO->getLeaveCoverage($lai);
				...
				foreach($getLeaveCoverage as $key=>$val)
				{
					...
					$getDTR = $timesheetDAO->getDTR($empidLC,$monthFrom, $dayFrom, $yearFrom);
				...
				// echo "</pre>";
					...
					// $saveLeaveStatusTimeLog = $leaveApplicationDAO->saveLeaveStatusTimeLog($emp_id,$getDtrID,$emp_timein,$emp_timeout,$emp_totalhours,$emp_late,$emp_late,$emp_late,$emp_late,$status_id);
					$saveLeaveStatusTimeLog = $leaveApplicationDAO->saveLeaveStatusTimeLog($emp_id,$getDtrID,$emp_timein,$emp_timeout,$emp_totalhours,$emp_late,$emp_late,$emp_late,$emp_late,$status_id_val);
					
					// $updateLeaveStatusTimeLog = $leaveApplicationDAO->updateLeaveStatusTimeLog($getDtrID,$status_id,$emp_totalhours,$emp_timein,$emp_timeout,$emp_late);
					$updateLeaveStatusTimeLog = $leaveApplicationDAO->updateLeaveStatusTimeLog($getDtrID,$status_id_val,$emp_totalhours,$emp_timein,$emp_timeout,$emp_late);
					// echo "<br/>";
