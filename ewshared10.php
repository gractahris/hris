...
class cMenu {
	...
	function AddMenuItem($id, $text, $url, $parentid = -1, $src = "", $allowed = TRUE, $grouptitle = FALSE, $customurl = FALSE) {
		...
		//Tolits Customization jfsbaldo 08202014
		if ($url==""){
			switch ($text)
			{
			case "Dashboard":
			  $text="<i class='icon-dashboard'></i> <span class='menu-text'>".$text."</span>";
			  break;
			case "Self Service Menu":
			  $text="<i class='icon-pencil'></i> <span class='menu-text'>".$text."</span>";
			  break;
			case "Recruitment":
			  $text="<i class='icon-folder-close'></i> <span class='menu-text'>".$text."</span>";
			  break;
			case "Personnel":
			  $text="<i class='icon-envelope'></i> <span class='menu-text'>".$text."</span>";
			  break;
			case "CENOPAC Tagging":
			  $text="<i class='icon-thumbs-up'></i> <span class='menu-text'>".$text."</span>";
			  break;
			case "Reports":
			  $text="<i class='icon-print'></i> <span class='menu-text'>".$text."</span>";
			  break;
			case "Libraries":
			  $text="<i class='icon-list-alt'></i> <span class='menu-text'>".$text."</span>";
			  break;
			case "Location Libraries":
			  $text="<i class='icon-globe'></i> <span class='menu-text'>".$text."</span>";
			  break;
			case "Access Control":
			  $text="<i class='icon-lock'></i> <span class='menu-text'>".$text."</span>";
			  break;
			case "Change Password":
			  $text="<i class='icon-unlock'></i> <span class='menu-text'>".$text."</span>";
			  break;
			case "Logout":
			  $text="<i class='icon-user'></i> <span class='menu-text'>".$text."</span>";
			  break;
			 case "Download Manual":
			  $text="<i class='icon-question-sign'></i> <span class='menu-text'>".$text."</span>";
			  break;
			default:
			  $text="<i class='icon-double-angle-right'></i> <span class='menu-text'>".$text."</span>";
			  break;
			}
			$menu_class="class='dropdown-toggle' ";

					//$text="<i class='icon-text-width'></i> <span class='menu-text'>".$text."</span>";
			}else{
			switch ($text)
			{
			case "Dashboard":
	...
	// get obsu_head and directorate head from users
	// modified by jfsbaldo 02282014 EDITZ
	function getEmpIDByUid($CurrentUID){
		global $conn;
		
		$sql = "SELECT emp_id from users
					WHERE
					uid='$CurrentUID'";
		$res = $conn->Execute($sql);
		$empid = $res->fields('emp_id');
		return $empid;
	}

	function getItemNoByEmpID($EmpID){
		global $conn;
		
		$sql = "SELECT ItemNo from employee
					WHERE
					EmpID='$EmpID'";
		$res = $conn->Execute($sql);
		$ItemNo = $res->fields('ItemNo');
		return $ItemNo;
	}

	function getPosTypeByItemNo($ItemNo){
		global $conn;
		
		$sql = "SELECT PositionType from lib_plantilla
					WHERE
					ItemNo='$ItemNo'";
		$res = $conn->Execute($sql);
		$PositionType = $res->fields('PositionType');
		return $PositionType;
	}

	function getObsuHeadFromUsers($CurrentUID){
		global $conn;
		
		$sql = "SELECT obsu_head from users
						WHERE
						uid='$CurrentUID'";
		$res = $conn->Execute($sql);
		$obsu_head = $res->fields('obsu_head');
		return $obsu_head;
	}
	
	function getDirectorateHeadFromUsers($CurrentUID){
		global $conn;
		
		$sql = "SELECT directorate_head from users
						WHERE
						uid='$CurrentUID'";
		$res = $conn->Execute($sql);
		$directorate_head = $res->fields('directorate_head');
...
function ew_HtmlEncode($exp) {
	$enye = '/^ñÑ/';
	$returnPregMatch = preg_match($enye,$exp);
	if ($returnPregMatch == 0) {
		return htmlspecialchars(strval($exp));
	} elseif ($returnPregMatch == 1) {
		return strval($exp);
	} elseif ($returnPregMatch == FALSE) {
		return htmlspecialchars(strval($exp));
	}
}
