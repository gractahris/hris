...
$tbl_employee_leavecredit_grid = NULL; // Initialize page object first

class ctbl_employee_leavecredit_grid extends ctbl_employee_leavecredit {
	...
	// Page object name
	var $PageObjName = 'tbl_employee_leavecredit_grid';

	// Grid form hidden field names
	var $FormName = 'ftbl_employee_leavecreditgrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["tbl_employee_leavecredit"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
		...
		for ($rowindex = 1; $rowindex <= $rowcnt; $rowindex++) {
			...
			if (!$this->EmptyRow()) {
				...
				if ($bGridInsert) {
					...
					$sKey .= $this->emp_leave_credit_id->CurrentValue;
	...
	function EmptyRow() {
		...
		if ($objForm->HasValue("x_emp_id") && $objForm->HasValue("o_emp_id") && $this->emp_id->CurrentValue <> $this->emp_id->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_leave_type_id") && $objForm->HasValue("o_leave_type_id") && $this->leave_type_id->CurrentValue <> $this->leave_type_id->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_leave_credit") && $objForm->HasValue("o_leave_credit") && $this->leave_credit->CurrentValue <> $this->leave_credit->OldValue)
			return FALSE;
		return TRUE;
	}
	...
	function RenderListOptions() {
		...
		if ($this->CurrentMode == "edit" && is_numeric($this->RowIndex)) {
			$this->MultiSelectKey .= "<input type=\"hidden\" name=\"" . $KeyName . "\" id=\"" . $KeyName . "\" value=\"" . $this->emp_leave_credit_id->CurrentValue . "\">";
		}
	...
	function SetRecordKey(&$key, $rs) {
		...
		$key .= $rs->fields('emp_leave_credit_id');
	}
	...
	function LoadDefaultValues() {
		$this->emp_leave_credit_id->CurrentValue = NULL;
		$this->emp_leave_credit_id->OldValue = $this->emp_leave_credit_id->CurrentValue;
	...
	function LoadFormValues() {
		...
		// Load from form
		global $objForm;
		if (!$this->emp_leave_credit_id->FldIsDetailKey && $this->CurrentAction <> "gridadd" && $this->CurrentAction <> "add")
		...
		$this->leave_type_id->setOldValue($objForm->GetValue("o_leave_type_id"));
		if (!$this->leave_credit->FldIsDetailKey) {
			$this->leave_credit->setFormValue($objForm->GetValue("x_leave_credit"));
		}
		$this->leave_credit->setOldValue($objForm->GetValue("o_leave_credit"));
	}
	...
	function LoadOldRecord() {
		...
		if ($cnt >= 1) {
			if (strval($arKeys[0]) <> "")
				$this->emp_leave_credit_id->CurrentValue = strval($arKeys[0]); // emp_leave_credit_id
			else
				$bValidKey = FALSE;
		} else {
			$bValidKey = FALSE;
	...
	function SetUpMasterParms() {
		...
		if ($sMasterTblVar == "tbl_employee") {
			...
			if ($GLOBALS["tbl_employee"]->EventCancelled) $this->EventCancelled = TRUE;
		}
