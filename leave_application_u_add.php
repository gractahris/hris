...
<link rel="stylesheet" type="text/css" media="all" href="js_datepicker/jsDatePick_ltr.min.css" />
<script type="text/javascript" src="js_datepicker/jsDatePick.min.1.3.js"></script>
...
function loadDateTo()
{
	...
	// window.onload = loadDateFrom;
	// window.onload = loadDateTo;

</script>

<body onload= "loadDateFrom(); loadDateTo();">
<?php
$leaveApplicationDAO = new leaveApplicationDAO();

?>
<form method="GET" action="leave_application_u_add.php">
<table border="1" width="100%" class ="table table-striped table-bordered table-hover">
	
	<tr>
		<td colspan = 9>
		<b>
		<font color = "red">VACATION LEAVE should be applied 3 days prior to this date</font><br/>
		<font color = "red">SICK LEAVE can be applied before/prior to this date</font>
		</b>
		<?php
		$dateApplied = date("Y-m-d"); //carla
		echo "<input type= \"hidden\" name=\"date_applied\" id =\"date_applied\" value=\"$dateApplied\" / >";
	...
	<tr>
		<td>LeaveType</td>
		<td>
			<select name="getAllLeaveType" id = "getAllLeaveType">
			<option id="#" value="#">Please Select</option>
			<?php
			
			$getAllLeaveType = $leaveApplicationDAO->getAllLeaveType();

			foreach ($getAllLeaveType as $keyLT => $valLT) {
				echo "<option id=";
				echo $valLT['leave_type_id'];
				echo " value =";
				echo $valLT['leave_type_id'];
				echo ">";
	...
	*/
	...
	<tr>
		<td>Status</td>
		<td>
			<select name="getAllStatus" id = "getAllStatus">
			<!-- <option id="#" value="#">Please Select</option> -->
			<?php
			$getAllStatus = $leaveApplicationDAO->getAllStatus();

			foreach ($getAllStatus as $keyStatus=> $valStatus) {
				echo "<option id=";
				echo $valStatus['status_id'];
				echo " value =";
				echo $valStatus['status_id'];
				echo ">";
				echo $valStatus['status_title'];
				echo "</option>";
			}
	...
	<tr>
		<td colspan = "2">
		<center>
		<input  class = "btn btn-sm btn-warning" type="submit" id="btnAdd" name="btnAdd" value = "ApplyLeave" />
		</center>
		</td>
	</tr>
...
<?php
$date_applied = $_GET['date_applied'];
$getAllLeaveTypeVal = $_GET['getAllLeaveType'];
$purposeVal = $_GET['purpose'];
$daysOfLeaveVal = $_GET['daysOfLeave'];
$dateFromVal = $_GET['dateFrom'];
$dateToVal=$_GET['dateTo'];
$place_to_visit = "NULL";
$getAllStatus = $_GET['getAllStatus'];

$getMaxLeaveApplicationID = $leaveApplicationDAO->getMaxLeaveApplicationID();
$maxID = $getMaxLeaveApplicationID[0]['maxLeaveApplicationID'];
// echo $maxID;

if($_GET['btnAdd'] = "ApplyLeave")
{
	//if($daysOfLeaveVal <= 0)
	//{
		?>
		<script>
		//alert("Enter NUMBER of Day!");
		</script>
		<?php
	//}else
	//{
		// $dateFrom = $_GET['dateFrom'];
		$arrOfDateFrom = explode('-', $dateFromVal);

		// $dateTo = $_GET['dateTo'];
		$arrOfDateTo = explode('-', $dateToVal);
		
		$yearFrom = $arrOfDateFrom[0];
		$yearTo = $arrOfDateTo[0];

		$monthFrom = $arrOfDateFrom[1];
		$monthTo = $arrOfDateTo[1];

		$dayFrom = $arrOfDateFrom[2];
		$dayTo = $arrOfDateTo[2];
		
		// echo "<br/>";
		// echo "yearFrom = ".$yearFrom;
		// echo "<br/>";
		// echo "yearTo = ".$yearTo;

		// echo "<br/>";
		// echo "monthFrom = ".$monthFrom;
		// echo "<br/>";
		// echo "monthTo = ".$monthTo;

		// echo "<br/>";
		// echo "DayFrom = ".$dayFrom;
		// echo "<br/>";
		// echo "DayTo = ".$dayTo;
		// echo "<br/>";
		...
		{
			$getLeaveDateDiff = $leaveApplicationDAO->getLeaveDateDiff($_GET['dateFrom']);
			$dateDiff = $getLeaveDateDiff[0]['dateDiff'];

			if($_GET['getAllLeaveType'] == 4)
			{
				...
				if($dateDiff < 3)
				{
					...
					?>
					<script>
						alert("Vacation Leave should be applied at least 3 days prior to this date !");
					</script>
					<?php

					echo "<b>Vacation Leave should be applied at least 3 days prior to this date !</b>";
				}else
				{

					$dayDiff = ($dayTo - $dayFrom) + 1;
			echo "Days of Leave Applied = ".$dayDiff;
			echo "<br/>";
			echo "Leave Coverage From = ".$_GET['dateFrom'];
			echo "<br/>";
			echo "Leave Coverage To = ".$_GET['dateTo'];
			echo "<br/>";
			echo "Date Applied = ".$date_applied;
			echo "<br/>";

			$saveLeave = $leaveApplicationDAO->saveLeave($maxID,$emp_id,$getAllLeaveTypeVal,$purposeVal,$place_to_visit,$dayDiff,$getAllStatus,$date_applied);

			echo "<br/>";
			for($counter = 0; $counter < $dayDiff; $counter++)
			{
				$dayModify = $dayFrom + $counter;
				if($dayModify <= 9)
				{
				...
				$dateModify = $yearFrom . "-". $monthFrom . "-" . $dayModify;
				// $saveLeaveCoverage = $leaveApplicationDAO->saveLeaveCoverage($maxID,$emp_id,$dateFromVal,$dateToVal);
				$saveLeaveCoverage = $leaveApplicationDAO->saveLeaveCoverage($maxID,$emp_id,$dateModify,$dateModify);echo "<br/>";
			}
			...
			<script>
			alert("Add Succeed");
			// window.location.href = "leave_application_u.php";
			</script>
			<?php
