...
$other_deduction_id = $_GET['other_deduction_id'];
echo "<input type=\"hidden\" value=\"$other_deduction_id\" id=\"other_deduction_id\" name=\"other_deduction_id\" />";


$getOtherDeduction = $employeeDeductionDAO->getOtherDeduction($other_deduction_id);

foreach($getOtherDeduction as $key => $val)
{

	// $getLeaveCreditBal = $leaveCreditDAO->getLeaveCreditBal($emp_id);
	// print_r($getLeaveCreditBal);
	$emp_id = $val['emp_id'];
	$month_val_OD = $val['month'];
	$year_val_OD = $val['year'];
	$deduction_title = $val['deduction_title'];
	$deduction_amount = $val['deduction_amount'];
	$cut_off_id_OD = $val['cut_off_id'];
	// echo $cut_off_id_OD;
	
	echo "<input type=\"hidden\" value=\"$emp_id\" id=\"emp_id\" name=\"emp_id\" />";
	echo "<table class = \"table table-striped table-bordered table-hover\" align = \"center\" width=\"100%\" border = \"1\">";
	echo "<tr>";
		...
		echo "<td>";
		// echo "";
		$getAllMonth = $employeeDeductionDAO->getAllMonth();
		echo "<select id=\"month_id\" name=\"month_id\">";
		echo "<option id=\"#\" value = \"#\">Please Select Month</option>";
		foreach($getAllMonth as $key=>$val)
		{
			$month_id = $val['month_id'];
			$month_val = $val['month_val'];
			$month_title = $val['month_title'];

			if($month_val_OD == $month_val)
			{
				$select = "selected";
			}else
			{
			...
			// echo "<option value=\"".$leave_type_id_val."\" >".$leave_type_title."</option>";
				echo "<option id='";
				echo $month_val;
				echo "' value='";
				echo $month_val;
				echo "' ".$select." >".$month_title."</option>";
		}
		echo "</select>";
		echo "</td>";
	echo "</tr>";
	...
	echo "<tr>";
		...
		echo "<td>";
		// echo "";
		$getAllYear = $employeeDeductionDAO->getAllYear();
		echo "<select id=\"year_id\" name=\"year_id\">";
		echo "<option id=\"#\" value = \"#\">Please Select Year</option>";
		foreach($getAllYear as $key=>$val)
		{
			$year_id = $val['year_id'];
			$year_val = $val['year_val'];

			if($year_val_OD == $year_val)
			{
				$select = "selected";
			}else
			{
	...
	echo "<td>";
	// echo "";
	$getAllCutOff = $employeeDeductionDAO->getAllCutOff();

	echo "<select id=\"cut_off_id\" name=\"cut_off_id\">";
	echo "<option id=\"#\" value = \"#\">Please Select Cut Off</option>";
	foreach($getAllCutOff as $key=>$val)
	{
		$cut_off_id = $val['cut_off_id'];
		$cut_off_title = $val['cut_off_title'];
		// $month_title = $val['month_title'];
		// echo "<option value=\"".$leave_type_id_val."\" >".$leave_type_title."</option>";

		if($cut_off_id_OD == $cut_off_id)
			{
				$select = "selected";
			}else
			{
			...
			echo "' value='";
			echo $cut_off_id;
			echo "' ".$select." >".$cut_off_title."</option>";
	}
	echo "</select>";
	echo "</td>";
echo "</tr>";
	...
	echo "<tr>";
		...
		echo "<td>";
		echo "<input type=\"text\" id=\"d_title\" name=\"d_title\" value=\"$deduction_title\" />";
		echo "</td>";
	echo "</tr>";
	...
	echo "<tr>";
		...
		echo "<td>";
		echo "<input type=\"text\" id=\"d_amount\" name=\"d_amount\" value = \"$deduction_amount\"/>";
		echo "</td>";
	echo "</tr>";
	...
	echo "<tr>";
		echo "<td colspan = \"2\">";
		echo "<center>";
		echo "<input type=\"submit\" name=\"btnAdd\" id=\"btnAdd\" value=\"Update Deduction\">";
		echo "</center>";
		echo "</td>";
	echo "</tr>";
...
if(isset($_GET['btnAdd']))
{
	...
	$d_title = $_GET['d_title'];
	$d_amount = $_GET['d_amount'];
	$emp_id = $_GET['emp_id'];

	if($month_id == "" || $month_id  == NULL || $month_id  == "#")
	{
		?>
			<script>
			alert("Enter Month");
	...
	if($year_id == "" || $year_id  == NULL || $year_id  == "#")
	{
		?>
			<script>
			alert("Enter Year");
	...
	if($d_title == "" || $d_title  == NULL)
	{
		?>
			<script>
			alert("Enter Deduction Title");
			</script>
		<?php
	}

	if($d_amount == "" || $d_amount  == NULL)
	{
		?>
			<script>
	...
	if($year_id  <> "#" && $month_id  <> "#" && $d_title <> "" && $d_amount <> "" && is_numeric($d_amount))
	{
		$editOD = $employeeDeductionDAO->updateOtherDeduction($other_deduction_id,$d_title,$d_amount,$month_id,$year_id);
		?>
