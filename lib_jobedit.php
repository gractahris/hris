...
<?php
$jobDAO = new jobDAO();
$getJobByID = $jobDAO->getJobByID($_GET['job_id']);
...
<?php
foreach($getJobByID as $key => $val)
{

	$job_id = $val['job_id'];
	echo "<input type=\"hidden\" size=\"60\" id=\"job_id\" name = \"job_id\" value = \"".$job_id."\" />";
	echo "<tr>";
		echo "<td>JOB TITLE</td>";
		echo "<td><input type=\"text\" size=\"60\" id=\"jobTitle\" name = \"jobTitle\" value = \"".$val['job_title']."\" /></td>";
	echo "</tr>";


	echo "<tr>";
		echo "<td>JOB DESCRIPTION</td>";
		echo "<td><input type=\"text\" size=\"60\" id=\"jobDesc\" name = \"jobDesc\" value = \"".$val['job_desc']."\" /></td>";
	echo "</tr>";

	echo "<tr>";
		echo "<td>JOB SALARY</td>";
		echo "<td>";
		$getAllSalary = $jobDAO->getAllSalary();
		echo "<select name = \"salary_id\">";
		echo "<option value=\"#\">Please select salary</option>";
		
			$getSalaryByID = $jobDAO->getSalaryByID($val['salary_id']);
			
			foreach($getAllSalary as $keySal => $valSal)
			{
				
				echo $valSal['salary_id'];
				echo "<option value=\" " . $valSal['salary_id'] . "\"";
				if($val['salary_id'] == $valSal['salary_id'])
				{
					echo " SELECT ";
				}else
				{
	...
	echo "<tr>";
	echo "<td  colspan = \"2\" ><input class=\"btn btn-sm btn-purple\" type=\"submit\" id=\"btnEdit\" name = \"btnEdit\" value = \"Update Job\"></td>";
	echo "</tr>";
...
if(isset($_GET['btnEdit']))
{
	...
	$job_desc = $_GET['jobDesc'];
	$salary_id = $_GET['salary_id'];

	$editJob = $jobDAO->editJob($job_title,$job_desc,$job_id,$salary_id);

	?>

		<script type="text/javascript">
		alert("Update Succeeded !");
		window.location.href = "lib_joblist.php";
		</script>
	<?php
}
