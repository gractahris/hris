...
$emp_leave_credit_id = $_GET['emp_leave_credit_id'];
echo "<input type=\"hidden\" value=\"$emp_leave_credit_id\" id=\"emp_leave_credit_id\" name=\"emp_leave_credit_id\" />";
...
$getLeaveCreditByID = $leaveCreditDAO->getLeaveCreditByID($emp_leave_credit_id);

foreach($getLeaveCreditByID as $keyLID => $valLID)
{
	$glcTypeID = $valLID['leave_type_id'];
	$glcLeaveCredit = $valLID['leave_credit'];
}


	$getAllLeaveType = $leaveCreditDAO->getAllLeaveType();
	echo "<select id=\"leave_type_id\" name=\"leave_type_id\">";
	echo "<option id=\"#\" value = \"#\">Please Select Leave Type</option>";
	foreach($getAllLeaveType as $key=>$val)
	{


		$leave_type_id_val = $val['leave_type_id'];
		$leave_type_title_val = $val['leave_type_title'];
		// echo "<option value=\"".$leave_type_id_val."\" >".$leave_type_title."</option>";

		// $select = "";
		if($glcTypeID == $leave_type_id_val)
		{
			$select = "selected";
		}else
		{
			...
			echo "<option id='";
			echo $leave_type_id_val;
			echo "' value='";
			echo $leave_type_id_val;
			echo "' ".$select." >".$leave_type_title_val."</option>";
...
echo "<tr>";
	...
	echo "<td>";
	echo "<input type=\"text\" id=\"leave_credit\" name=\"leave_credit\" value=\"$glcLeaveCredit\" />";
	echo "</td>";
echo "</tr>";
...
echo "<tr>";
	echo "<td colspan = \"2\">";
	echo "<center>";
	echo "<input type=\"submit\" name=\"btnAdd\" id=\"btnAdd\" value=\"Update Leave\">";
	echo "</center>";
	echo "</td>";
echo "</tr>";
...
{
	$emp_leave_credit_id=$_GET['emp_leave_credit_id'];
	$leave_credit=$_GET['leave_credit'];
	$leave_type_id=$_GET['leave_type_id'];

	if($leave_credit == "" || $leave_credit  == NULL)
	{
		?>
			<script>
			alert("Enter Leave Credit");
			</script>
		<?php
	}
	...
	if($leave_type_id <> "" || $leave_type_id  <> NULL || $leave_credit <> "" || $leave_credit  <> NULL)
	{
		$leaveCreditDAO->updateLeaveType($emp_leave_credit_id,$leave_type_id,$leave_credit);
