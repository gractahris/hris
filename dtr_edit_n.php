...
<?php include_once "model/editDTRDAO.php" ?>
...
// echo "EmpID=>".$emp_id."<br/>";
// echo "dtrID=>".$dtr_id."<br/>";

$editDTRDAO = new editDTRDAO();

$getEmpTimeLog = $editDTRDAO->getEmpTimeLog($emp_id,$dtr_id);

// print_r($getEmpTimeLog);
?>
<form method="GET">
<table class = "table table-hover" width = "100%" height = "100%">

	<input  type="hidden" size="60" id = "emp_id" name="emp_id" value="<?php echo $emp_id ?>">
	<input  type="hidden" size="60" id = "dtr_id" name="dtr_id" value="<?php echo $dtr_id ?>">


	<tr>
		<td>Time IN</td>
		<td><input  type="time" size="60" id = "txtTimeIn" name="txtTimeIn" value="<?php echo $getEmpTimeLog[0]['emp_timein'] ?>" /></td>
	</tr>

	<tr>
		<td>Time out</td>
		<td><input  type="time" size="60" id = "txtTimeOut" name="txtTimeOut" value="<?php echo $getEmpTimeLog[0]['emp_timeout'] ?>" /></td>
	</tr>
	<tr>
		<td colspan = 2>
...
{
	if($_GET['txtTimeOut'] < $_GET['txtTimeIn'])
	{
		?>
			<h3>Time out should not be greater than time in</h3>
<?php
	}

		else
	{
		$updateTimeLogs = $editDTRDAO->updateTimeLogs($_GET['emp_id'], $_GET['dtr_id'], $_GET['txtTimeOut'], $_GET['txtTimeIn']);
		?>
		<script>
